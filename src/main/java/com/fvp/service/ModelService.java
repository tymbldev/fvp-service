...
public class ModelService {
    ...
    public List<ModelWithLinkDTO> getAllModels(Integer tenantId) {
        ...
        return LoggingUtil.logOperationTime(logger, "get home models", () -> {
            ...
            if (!modelDTOs.isEmpty()) {
                cacheService.putInCacheWithExpiry(CacheService.CACHE_NAME_MODELS, cacheKey, modelDTOs, CACHE_EXPIRY_MINUTES, TimeUnit.MINUTES);
            }
            return modelDTOs;
        });
    }
    ...
    public ModelWithLinkDTO getModelFirstLink(Integer tenantId, String modelName) {
        ...
        return LoggingUtil.logOperationTime(logger, "get model first link", () -> {
            ...
            if (model != null) {
                cacheService.putInCacheWithExpiry(CacheService.CACHE_NAME_MODELS, cacheKey, model, CACHE_EXPIRY_MINUTES, TimeUnit.MINUTES);
            }
            return model;
        });
    }
