...
public interface LinkModelShard5Repository extends ShardedLinkModelRepository<LinkModelShard5> {
  ...
  @Query(value = "SELECT lm.* FROM link_model_shard_5 lm JOIN link l ON lm.link_id = l.id " +
      ...
      nativeQuery = true)
  List<LinkModelShard5> findByModelWithFiltersPageable(
      @Param("tenantId") Integer tenantId,
      @Param("model") String model,
  ...
  @Query(value = "SELECT lm.* FROM link_model_shard_5 lm JOIN link l ON lm.link_id = l.id " +
      ...
      "ORDER BY lm.random_order LIMIT :limit OFFSET :offset",
      nativeQuery = true)
  List<LinkModelShard5> findByModelWithFiltersExcludingLinkPageable(
      @Param("tenantId") Integer tenantId,
      @Param("model") String model,
      @Param("maxDuration") Integer maxDuration,
      @Param("quality") String quality,
  ...
  @Modifying
  @Transactional
  @Query(value = "DELETE FROM link_model_shard_5 WHERE link_id = :linkId", nativeQuery = true)
