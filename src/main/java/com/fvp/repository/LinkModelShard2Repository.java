...
public interface LinkModelShard2Repository extends ShardedLinkModelRepository<LinkModelShard2> {
  ...
  @Query(value = "SELECT lm.* FROM link_model_shard_2 lm JOIN link l ON lm.link_id = l.id " +
      ...
      nativeQuery = true)
  List<LinkModelShard2> findByModelWithFiltersPageable(
      @Param("tenantId") Integer tenantId,
      @Param("model") String model,
  ...
  @Override
  @Query(value = "SELECT DISTINCT lm.model FROM link_model_shard_2 lm " +
  ...
  @Query(value = "SELECT lm.* FROM link_model_shard_2 lm JOIN link l ON lm.link_id = l.id " +
      ...
      "ORDER BY lm.random_order LIMIT :limit OFFSET :offset",
      nativeQuery = true)
  List<LinkModelShard2> findByModelWithFiltersExcludingLinkPageable(
      @Param("tenantId") Integer tenantId,
      @Param("model") String model,
      @Param("maxDuration") Integer maxDuration,
      @Param("quality") String quality,
  ...
  @Override
  @Query(value = "SELECT COUNT(lm.id) FROM link_model_shard_2 lm JOIN link l ON lm.link_id = l.id "
      +
      "WHERE lm.tenant_id = :tenantId AND lm.model = :model " +
      "AND (:maxDuration IS NULL OR l.duration <= :maxDuration) " +
      "AND (:quality IS NULL OR :quality = '' OR l.quality = :quality) " +
  ...
  @Modifying
  @Transactional
  @Query(value = "DELETE FROM link_model_shard_2 WHERE link_id = :linkId", nativeQuery = true)
