...
public interface LinkCategoryShard9Repository extends
  ...
  @Query(value =
      "SELECT COUNT(lc.id) AS count FROM link_category_shard_9 lc JOIN link l ON lc.link_id = l.id "
          +
          "WHERE lc.tenant_id = :tenantId AND lc.category = :category " +
          "AND l.thumb_path_processed = 1",
      nativeQuery = true)
  ...
  @Query(value =
      "SELECT lc.category, COUNT(lc.id) as count FROM link_category_shard_9 lc JOIN link l ON lc.link_id = l.id "
          +
