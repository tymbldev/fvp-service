...
public interface LinkModelShard3Repository extends ShardedLinkModelRepository<LinkModelShard3> {
  ...
  @Query(value = "SELECT lm.* FROM link_model_shard_3 lm JOIN link l ON lm.link_id = l.id " +
      ...
      nativeQuery = true)
  List<LinkModelShard3> findByModelWithFiltersPageable(
      @Param("tenantId") Integer tenantId,
      @Param("model") String model,
  ...
  @Override
  @Query(value = "SELECT DISTINCT lm.model FROM link_model_shard_3 lm " +
  ...
  @Query(value = "SELECT lm.* FROM link_model_shard_3 lm JOIN link l ON lm.link_id = l.id " +
      ...
      "ORDER BY lm.random_order LIMIT :limit OFFSET :offset",
      nativeQuery = true)
  List<LinkModelShard3> findByModelWithFiltersExcludingLinkPageable(
      @Param("tenantId") Integer tenantId,
      @Param("model") String model,
      @Param("maxDuration") Integer maxDuration,
      @Param("quality") String quality,
  ...
  @Override
  @Query(value = "SELECT COUNT(lm.id) FROM link_model_shard_3 lm JOIN link l ON lm.link_id = l.id " +
      "WHERE lm.tenant_id = :tenantId AND lm.model = :model " +
      "AND (:maxDuration IS NULL OR l.duration <= :maxDuration) " +
      "AND (:quality IS NULL OR :quality = '' OR l.quality = :quality) " +
      "AND l.thumb_path_processed = 1",
  ...
  @Modifying
  @Transactional
  @Query(value = "DELETE FROM link_model_shard_3 WHERE link_id = :linkId", nativeQuery = true)
