...
public interface LinkModelShard10Repository extends ShardedLinkModelRepository<LinkModelShard10> {
  ...
  @Override
  @Query(value = "SELECT lm.* FROM link_model_shard_10 lm JOIN link l ON lm.link_id = l.id " +
      "WHERE lm.tenant_id = :tenantId AND lm.model = :model " +
      ...
      nativeQuery = true)
  List<LinkModelShard10> findByModelWithFiltersPageable(
      @Param("tenantId") Integer tenantId,
      @Param("model") String model,
  ...
  @Override
  @Query(value = "SELECT DISTINCT lm.model FROM link_model_shard_10 lm " +
      "JOIN link l ON lm.link_id = l.id " +
  ...
  @Query(value = "SELECT lm.* FROM link_model_shard_10 lm JOIN link l ON lm.link_id = l.id " +
      ...
      "ORDER BY lm.random_order LIMIT :limit OFFSET :offset",
      nativeQuery = true)
  List<LinkModelShard10> findByModelWithFiltersExcludingLinkPageable(
      @Param("tenantId") Integer tenantId,
      @Param("model") String model,
      @Param("maxDuration") Integer maxDuration,
      @Param("quality") String quality,
  ...
  @Override
  @Query(value = "SELECT COUNT(*) FROM link_model_shard_10 lm JOIN link l ON lm.link_id = l.id " +
      "WHERE lm.tenant_id = :tenantId AND lm.model = :model " +
      "AND (:maxDuration IS NULL OR l.duration <= :maxDuration) " +
      "AND (:quality IS NULL OR :quality = '' OR l.quality = :quality) " +
      "AND l.thumb_path_processed = true",
  ...
  @Modifying
  @Transactional
  @Query(value = "DELETE FROM link_model_shard_10 WHERE link_id = :linkId", nativeQuery = true)
